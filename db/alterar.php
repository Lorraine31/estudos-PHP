...
//através desse codigo vou procurar la no formulario por id e assim conseguir encontrar o dado que quero alterar
if($_GET['codigo']) {//esse form tem método $_GET, portando no html dele preciso colocar get no method
    $sql = "SELECT * FROM cadastro WHERE id = ?";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("i", $_GET['codigo']);

    if($stmt->execute()) { //se o stmt exceutar com o valor verdade, ele executa o resto do if
        $resultado = $stmt->get_result();
        if($resultado->num_rows > 0) { //se as linhas forem maiores que zero, tem resultado e posso alterar o dado
            $dados = $resultado->fetch_assoc();
            if($dados['nascimento']) {//criando um if diferente ora data pq setamos ela usando parametros diferentes
...
if(count($_POST) > 0) {
    ...
    if(!count($erros)) {
        // cadastro com o update posso modificar os valores
        $sql = "UPDATE
        ...
        $params = [
            ...
            $dados['site'],
            $dados['filhos'],
            $dados['salario'] ? str_replace(",", ".", $dados['salario']) : null,//alterei aqui para conseguir aceitar . ou , no salario
            $dados['id'],
        ];
...
<!-- essa parte vai me retornar os valores referentes a cada id que escolhi -->
